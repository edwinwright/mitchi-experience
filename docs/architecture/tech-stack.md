...
Tailwind, with theme tokens declared in `globals.css` via `@theme`. Long-form copy uses the Tailwind Typography plugin (`prose` on `RichText`). Class merging through a local `cn()` helper built on `clsx` and `tailwind-merge`.
